...
- Claude Code [Hooks](https://docs.anthropic.com/en/docs/claude-code/hooks) 在任务完成/需要输入时触发，POST 到 CBuddy
- CBuddy 通过飞书 API 发消息，绑定 `msg_id <-> TTY`
- 用户回复消息，飞书通过 WebSocket 长连接推送到 CBuddy
- CBuddy 通过 AppleScript 把回复内容 keystroke 到对应 Terminal.app tab
